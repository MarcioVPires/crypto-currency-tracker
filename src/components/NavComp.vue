<template>
  <div class="w-full sm:w-1/4 flex flex-col justify-center sm:order-1">
    <div class="flex flex-col">
      <div class="flex items-center">
        <router-link
          :class="`${
            watchlistActive ? style.activeTxt : style.inactiveTxt
          } w-1/2 sm:hover:bg-[#21406C]`"
          @click="ToggleActive"
          to="/"
          >Watchlist</router-link
        >

        <router-link
          :class="`${
            !watchlistActive ? style.activeTxt : style.inactiveTxt
          } w-1/2 sm:hover:bg-[#21406C]`"
          @click="ToggleActive"
          to="/cryptocurrencies"
          >Currencies</router-link
        >
      </div>
      <div class="h-1.5 bg-[#3C516F] flex flex-row">
        <div
          :class="`h-full w-1/2 bg-[#6BA6FF] ${
            watchlistActive ? style.left : style.right
          } transition-all`"
        />
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "NavComp",
  data() {
    return {
      style: {
        activeTxt: "text-white font-bold",
        inactiveTxt: "text-[#DCDCDC] font-light",
        left: "ml-0",
        right: "ml-[50%]",
      },
      watchlistActive: true,
    };
  },
  methods: {
    ToggleActive() {
      this.watchlistActive = !this.watchlistActive;
    },
  },
};
</script>
